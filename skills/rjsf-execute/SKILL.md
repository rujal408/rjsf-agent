...
### Code Generation Order: Template-First Architecture (MANDATORY)

**Problem:** RJSF renders forms as flat, single-column, unstyled field lists by default. The ObjectFieldTemplate controls how every section and field group is laid out — it is the single highest-impact component for visual quality. When schema is written first, the template becomes a retrofit that patches layout after the fact, producing inconsistent spacing, alignment, and grouping.

**Rule: Generate code in this exact order — not alphabetically, not by file type:**

| Order | What | Why |
|-------|------|-----|
| **Step 5a** | `templates/SectionTemplate.tsx` (ObjectFieldTemplate) | The visual skeleton — every other component renders inside it. Defines grid layout, section cards, spacing, responsive breakpoints. |
| **Step 5b** | `rjsf-overrides.css` (base CSS) | Fixes RJSF defaults (centered labels, no borders, inconsistent spacing) to match the prototype. |
| **Step 5c** | Responsive Grid CSS (`FormGrid.module.css` or equivalent) | Establishes the column system the template uses. |
| **Step 5d** | Custom Widgets (`widgets/*.tsx`) | Single-input controls must match the visual density the template established (padding, fonts, borders, colors). |
| **Step 5e** | Custom Fields (`fields/*.tsx`) | Multi-input controls — same visual consistency requirement. |
| **Step 5f** | Custom Templates (other than SectionTemplate) | Additional layout templates (Wizard, Tabs, etc.) that build on the base grid. |
| **Step 5g** | `schema.ts` (JSON Schema) | Schema properties map to sections already defined in the ObjectFieldTemplate. No orphaned fields — every field has a visual home. |
| **Step 5h** | `uiSchema.ts` | `ui:widget` and `ui:field` references point to components that already exist. `ui:options` align with the grid system the template established. |
| **Step 5i** | `types.ts` (TypeScript interfaces) | Derived from schema — must come after schema. |
| **Step 5j** | `index.tsx` (assembly) | Wires everything together. Schema, uiSchema, custom components, and templates all exist by this point. |
| **Step 5k** | Error Display Configuration | Configures `showErrorList` and any custom `FieldTemplate`. |
| **Step 5l** | Edge Case Handlers | Applied only when flags are true. |

**Why this order matters:** When the template is designed first, the schema conforms to the visual structure rather than the visual structure being forced to accommodate an arbitrary schema shape. This eliminates the most common aesthetic failures: inconsistent section boundaries, misaligned columns, fields that don't visually belong together being grouped by data proximity rather than user task flow.

---

### 5a. `templates/SectionTemplate.tsx` — Grid Layout Template (MANDATORY — generate FIRST)

**Always generate this for any form, even single-column forms.** This is the form's visual skeleton — every section and field group renders through it. Add `ObjectFieldTemplate: SectionTemplate` to the Form's `templates` prop even if the FormPlan's Customization Assessment didn't list a custom ObjectFieldTemplate.
...
4. Apply section card styling:
   - `border: 1px solid #e2e8f0`
   - `border-radius: 8px`
   - `padding: 24px`
   - `margin-bottom: 24px`
5. Render section title as a styled legend/heading (font-size, font-weight, margin-bottom, optional description text)
6. Apply field spacing within sections (`gap: 16px` rows, `24px` columns)
7. Be responsive (collapse to 1 column on mobile, FormPlan columns on desktop)
8. Handle empty sections (hide sections with zero visible fields)
...
**Design the SectionTemplate by reading the approved prototype first.** Extract:
- Section card border, radius, padding, margin from `.section` elements
- Grid gap from `.grid-*` classes
- Section title styling (font-size, weight, color)
- Which fields are side-by-side vs full-width per section

The SectionTemplate IS the form's aesthetic foundation. Every widget, field, and schema property renders inside it. Get this right first, and the rest follows naturally.

---

### 5b. `rjsf-overrides.css` — Base CSS Overrides (MANDATORY — generate SECOND)
...
**Key overrides that MUST be included (use color tokens matching the SectionTemplate):**
1. Left-align all labels (`text-align: left`)
2. Consistent input sizing (min-height 44px, padding, border-radius)
3. Focus ring styling (`#3182ce` or matching `colorPalette.focus`)
4. Error field border (`#e53e3e` or matching `colorPalette.error`)
5. Help text styling (small, gray)
6. Error message styling (small, red)
7. Button right-alignment
8. Section fieldset border + border-radius + padding (consistent with SectionTemplate)
...
**Visual consistency requirement:** Each widget must match the visual density established by the SectionTemplate (Step 5a):
- Use the same color tokens (border: `#e2e8f0`, focus ring: `#3182ce`, error: `#e53e3e`)
- Match padding, font sizes, and border styles from the prototype
- Meet 44px minimum touch target height (WCAG 2.5.5)
...
**Visual consistency requirement:** Same as widgets — match the SectionTemplate's visual density and color tokens.
...
### 5f. Custom Templates (other than SectionTemplate)

For each custom Template listed in the Customization Assessment (excluding SectionTemplate, which was generated in Step 5a), create `templates/<ComponentName>.tsx` following the relevant template interface (ObjectFieldTemplateProps, ArrayFieldTemplateProps, or FieldTemplateProps).

These templates build on the base grid system established in Steps 5a–5c. They must use the same spacing tokens and responsive breakpoints.
...
### 5g. `schema.ts` — JSON Schema

**Generate schema AFTER templates and widgets exist.** Schema properties must map to sections already defined in the SectionTemplate. Every field must have a visual home — no orphaned properties.
...
export const schema: RJSFSchema = {
  ...
  properties: {
    ...
    // Section grouping MUST match the SectionTemplate's config object.
...
### 5h. `uiSchema.ts` — UI Schema

**Generate uiSchema AFTER widgets and fields exist.** `ui:widget` and `ui:field` references must point to components that were already generated in Steps 5d–5f. `ui:options` (e.g., `classNames`, column spans) must align with the grid system the SectionTemplate established.
...
### 5j. `index.tsx` — Form Component (Assembly)

**This is the final assembly step.** By this point, SectionTemplate, CSS overrides, widgets, fields, schema, uiSchema, and types all exist. Wire them together.
...
import type { <FormName>Data } from './types';
// Import SectionTemplate (ALWAYS — generated in Step 5a):
import { SectionTemplate } from './templates/SectionTemplate';
// Import CSS overrides (ALWAYS — generated in Step 5b):
import './rjsf-overrides.css';  // or the appropriate import for styling approach
...
// ALWAYS include SectionTemplate as ObjectFieldTemplate.
// Add other templates only if listed in requiredTemplates.
const templates = {
  ObjectFieldTemplate: SectionTemplate,
  // e.g., ArrayFieldTemplate: SortableArrayTemplate — only if in requiredTemplates
};
...
### 5k. Error Display Configuration
  ...
  return (
    <div className="rjsf-form-card">
      ...
      <Form
        ...
        omitExtraData={false}
        // --- Error display: same logic as single-page form (see 5k) ---
...
- [ ] **Template-first order followed:** SectionTemplate (5a) → CSS overrides (5b) → Grid CSS (5c) → Widgets (5d) → Fields (5e) → Templates (5f) → Schema (5g) → uiSchema (5h) → Types (5i) → index.tsx (5j)
- [ ] Custom ObjectFieldTemplate (`SectionTemplate.tsx`) generated FIRST with grid layout matching FormPlan columns
...
- [ ] Section cards have border (`1px solid #e2e8f0`), border-radius (`8px`), padding (`24px`)
- [ ] Custom widgets/fields use same color tokens as SectionTemplate
...
- [ ] Schema section groupings match SectionTemplate config — no orphaned fields
...
├── templates/
│   ├── SectionTemplate.tsx      (MANDATORY — generated FIRST as visual skeleton)
│   └── <CustomTemplateName>.tsx  (if any)
├── rjsf-overrides.css          (MANDATORY — base CSS overrides)
├── FormGrid.module.css          (or equivalent for styling approach)
├── widgets/
│   └── <CustomWidgetName>.tsx   (if any — per Customization Assessment)
├── fields/
│   └── <CustomFieldName>.tsx    (if any)
├── schema.ts                    (generated AFTER templates/widgets)
├── uiSchema.ts                  (references existing widgets/fields)
├── types.ts                     (derived from schema)
└── index.tsx                    (final assembly — wires everything together)
```
